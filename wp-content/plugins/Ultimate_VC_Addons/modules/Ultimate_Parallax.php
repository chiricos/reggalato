	...
	class VC_Ultimate_Parallax{
		...
		public static function parallax_shortcode($atts, $content){
			...
			if((get_option('ultimate_row') == "enable" && $bg_type !== "") || $parallax_content != '' || $fadeout_row != ''){
				...
				if($ultimate_js != 'enable') :
					...
					{
						wp_enqueue_script('ultimate-row-bg',plugins_url('../assets/min-js/',__FILE__).'ultimate_bg.min.js?'.$bg_type);
				...
				$html = $autoplay = $muted = $loop = $pos_suffix = $bg_img = $bg_img_id = $icon_inline = $commom_data_attributes = '';
				...
				elseif($bg_type == "image"){
					...
					if($parallax_style == 'vcpb-fs-jquery' || $parallax_style=="vcpb-mlvp-jquery"){
						if($parallax_style == 'vcpb-fs-jquery')
							wp_enqueue_script('jquery.shake',plugins_url('../assets/min-js/jparallax.min.js',__FILE__));
						
						if($parallax_style=="vcpb-mlvp-jquery")
							wp_enqueue_script('jquery.vhparallax',plugins_url('../assets/min-js/jquery.vhparallax.min.js',__FILE__));
				...
				elseif ($bg_type=='u_iframe') {
					...
					wp_enqueue_script('jquery.ytplayer',plugins_url('../assets/min-js/jquery.mb.YTPlayer.min.js',__FILE__));
					$v_opts = explode(",",$video_opts);
					$v_img = wp_get_attachment_image_src($video_poster,'full');
					if(is_array($v_opts)){
						foreach($v_opts as $opt){
		...
		function parallax_init(){
			...
			if(function_exists('vc_add_param')){
				...
				vc_add_param('vc_row',array(
						...
						"value" => array(
								__("Move with the content", "ultimate_vc") => "scroll",
				...
				vc_add_param('vc_row',array(
						...
						"min"=>"1",
						"max"=>"100",
						"description" => __("Control speed of parallax. Enter value between 1 to 100", "ultimate_vc"),
				...
				vc_add_param('vc_row',array(
						...
						"value" => array(
								__("Left to Right", "ultimate_vc") => "left-animation",
				...
				vc_add_param('vc_row',array(
						...
						"value" => "30",
						"group" => $group_effects,
						"description" => __("Enter value between 0 to 100", "ultimate_vc"),
				...
				vc_add_param('vc_row',array(
						...
						"value" => "30",
						"group" => $group_effects,
						"description" => __("The area of screen from top where fade out effect will take effect once the row is completely inside that area.", "ultimate_vc"),
				...
				vc_add_param(
					...
					array(
						...
						'group' => $group_effects,
						'dependency' => Array('element' => 'enable_overlay', 'value' => array('enable_overlay_value')),
						'description' => __('Enter value between 0 to 100 (0 is maximum transparency, while 100 is minimum)','ultimate_vc'),
				...
				vc_add_param(
					...
					array(
						'type' => 'checkbox',
						'heading' => __('Fany Multi Color Overlay','ultimate_vc'),
				...
				vc_add_param(
					...
					array(
						...
						'value' => array(
							__('None','ultimate_vc') => 'none_seperator',
				...
				vc_add_param(
					...
					array(
						...
						'value' => array(
							__('Top','ultimate_vc') => 'top_seperator',
				...
				vc_add_param(
					...
					array(
						...
						'description' => __('Mostly, this should be background color of your adjacent row section.','ultimate_vc')
					)
				);
				vc_add_param(
					'vc_row',
		...
		static function front_scripts(){
			...
			if($ultimate_css != "enable")
				wp_enqueue_style('background-style',plugins_url('../assets/min-css/background-style.min.css',__FILE__));
		} /* end front_scripts */
	}
