...
if(!class_exists('AIO_creative_link')) 
{
...

	{
		function __construct()
		{
			...
			add_action( 'wp_enqueue_scripts', array( $this, 'link_admin_scripts') );
			//add_action( 'admin_enqueue_scripts', array( $this, 'link_backend_scripts') );	
		}

		//enque script
		function link_admin_scripts(){
	...
	if($link_hover_style!='Style_10'){
			...
			$output .='<span class=" ult_main_cl '.$el_class.' '.$style11_css_class.'" >
	 			<span class="'.$class.'  ult_crlink" >
					<a  href = "'.$url.'" '.$target.' class="ult_colorlink  '.$css_class .'" style="'.$colorstyle.' "  '.$data_link.'>
						'.$before.'
			...
			$output .='<span class=" ult_main_cl  '.$el_class.'" >
	 			<span class="'.$class.'  ult_crlink" id="'.$id.'">
					<a  href = "'.$url.'" '.$target.' class="ult_colorlink   "  style="'.$colorstyle.' "  '.$data_link.'>
					...
					array(
					   ...
					   "params" => array(							
							...
							array(
								"type" => "colorpicker",
								"class" => "",
								"heading" => __(" Border HoverColor", "ultimate_vc"),
